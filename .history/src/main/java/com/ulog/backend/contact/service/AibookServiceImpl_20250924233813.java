...
""";
    ...
    public AibookDto generate(String contactDesc, String userDesc, String language) {
        log.info("Generating aibook for contactDesc: {}, userDesc: {}, language: {}", 
                contactDesc != null ? contactDesc.substring(0, Math.min(50, contactDesc.length())) + "..." : "null",
                userDesc != null ? userDesc.substring(0, Math.min(50, userDesc.length())) + "..." : "null",
                language);
        
        if ((contactDesc == null || contactDesc.isBlank()) && (userDesc == null || userDesc.isBlank())) {
            log.warn("Both descriptions are empty");
...
""", nullToEmpty(contactDesc), nullToEmpty(userDesc));
        ...
        log.info("Calling Deepseek service...");
        try {
            String rawResponse = deepseekService.ask(systemPrompt, userPrompt)
                .timeout(Duration.ofSeconds(30))
                .block();
            
            log.info("Deepseek response received, length: {}", rawResponse != null ? rawResponse.length() : 0);
            ...
            log.info("Parsing aibook response...");
            AibookDto result = parseAibookResponse(rawResponse);
            log.info("Aibook generation completed successfully");
            return result;
